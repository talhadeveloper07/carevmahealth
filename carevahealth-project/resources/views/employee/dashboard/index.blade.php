    ...
    <div class="card-body">
        ...
        <div class="mt-4 d-flex justify-content-center">
            <svg id="progressCircle" width="180" height="180" viewBox="0 0 120 120">
                ...
                <!-- Time Text -->
                <text id="progressText" x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" font-size="18" fill="#333">
                    00:00
                </text>
            </svg>
        </div>
            ...
            <p id="workedTime">
                @if($attendanceToday && $attendanceToday->clock_in)
                    @if($attendanceToday->clock_out)
                        {{ \Carbon\Carbon::parse($attendanceToday->clock_in)->diff(\Carbon\Carbon::parse($attendanceToday->clock_out))->format('%H:%I') }} hrs
                    @else
                        Running...
                    @endif
                @else
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    let breakLimit = @json($attendanceToday?->employee->break_allowed_hours ?? 0); 
    // e.g. "02:00:00" from DB
    ...
    function updateProgress(diffSeconds) {
        // let shiftSeconds = 9 * 3600; 
        let shiftSeconds = 2 * 3600;
        let percent = Math.min((diffSeconds / shiftSeconds) * 100, 100);
        let offset = circleLength - (circleLength * percent / 100);
        path.setAttribute("stroke-dashoffset", offset);

        let hours = String(Math.floor(diffSeconds / 3600)).padStart(2, '0');
        let minutes = String(Math.floor((diffSeconds % 3600) / 60)).padStart(2, '0');
        text.textContent = `${hours}:${minutes}`;
        workedTime.innerText = `${hours}:${minutes} hrs`;
    }

    // Start main work timer
    ...
    // ✅ Start break countdown (HH:MM:SS safe)
    function startBreakTimer(limitTime) {
        ...
        let remaining = (hours * 3600) + (minutes * 60) + seconds; // total seconds
        ...
        breakInterval = setInterval(() => {
            ...
            let h = String(Math.floor(remaining / 3600)).padStart(2, '0');
            let m = String(Math.floor((remaining % 3600) / 60)).padStart(2, '0');
    ...
    if (onBreak && breakLimit) {
        startBreakTimer(breakLimit); // pass "02:00:00" etc
    }
});
</script>
